...
class SaleOrder(models.Model):
    ...
    def _total_receivable(self):
        receivable = 0.0
        if self.partner_id:
            self._cr.execute("""SELECT sum(amount_residual_signed) FROM account_move WHERE partner_id = %s""", (self.partner_id.id,))
            outstaning_value = self._cr.fetchall()
            if outstaning_value[0][0] is not None:
    ...
    def onchange_order_line(self):
        ...
        if self.discount_type == 'fixed' and amount_total:
    ...
    def action_confirm(self):
        for line in self.order_line:
            ...
            if line.product_uom_qty <= 0:
                raise UserError(_("Quantity for %s is %s. Please update the quantity or remove the product line.") % (line.product_id.name, line.product_uom_qty))
            if bool(self.env['ir.config_parameter'].sudo().get_param('bahmni_sale.is_delivery_automated')):
                if line.product_id.tracking == 'lot' and not line.lot_id:
                    raise UserError(_("Kindly choose batch no for %s to proceed further.") % line.product_id.name)

            if 1 < self.order_line.search_count([('lot_id', '=', line.lot_id.id), ('order_id', '=', self.id)]) and line.lot_id:
                raise UserError(_("%s Duplicate batch no is not allowed. Kindly change the batch no to proceed further.") % line.lot_id.name)
            if line.product_uom_qty > line.lot_id.product_qty and line.lot_id:
                raise UserError(_("Insufficient batch(%s) quantity for %s and available quantity is %s") % (line.lot_id.name, line.product_id.name, line.lot_id.product_qty))
            ...
            if order.picking_ids and bool(self.env['ir.config_parameter'].sudo().get_param('bahmni_sale.is_delivery_automated')):
                for picking in self.picking_ids:
                    ...
                    for mv_line in picking.move_ids.mapped('move_line_ids'):
                        if not mv_line.quantity and (mv_line.reserved_qty or mv_line.reserved_uom_qty):
                            mv_line.quantity = mv_line.reserved_qty or mv_line.reserved_uom_qty
                    ...
                    if picking.state not in ('waiting', 'confirmed', 'partially_available'):
                        for pack in picking.move_line_ids:
                            if pack.product_id.tracking != 'none':
                                ...
                                lot_ids = line.lot_id or self._find_batch(pack.product_id, pack.quantity, pack.location_id, picking)
                                if lot_ids:
                        ...
                        message = _("<b>Auto validation Failed</b> <br/> <b>Reason:</b> There are not enough stock available for Some product on Location: %s") % self.location_id.name
                        self.message_post(body=message)

    def _find_batch(self, product, qty, location, picking):
        _logger.info("\n\n***** Product :%s, Quantity :%s Location :%s\n*****", product, qty, location)
        lot_objs = self.env['stock.lot'].search([
            ('product_id', '=', product.id),
            ('expiration_date', '>=', fields.Datetime.now())
        ])
        _logger.info('\n *** Searched Lot Objects:%s \n', lot_objs)
        if any(lot_objs):
            sorted_lot_list = sorted(list(lot_objs), key=lambda l: l.expiration_date or datetime.max)
            _logger.info('\n *** Sorted based on FEFO :%s \n', sorted_lot_list)
            done_qty = qty
            res_lot_ids = []
            lot_ids_for_query = tuple([lot.id for lot in sorted_lot_list])
            self._cr.execute("SELECT SUM(quantity) FROM stock_quant WHERE lot_id IN %s and location_id=%s", (lot_ids_for_query, location.id,))
                ...
                for lot_obj in sorted_lot_list:
                    ...
                    for quant in quants:
                        if done_qty >= 0:
                            ...
                            done_qty = done_qty - quant.quantity
                return res_lot_ids
            else:
                message = _("<b>Auto validation Failed</b> <br/> <b>Reason:</b> There are not enough stock available for product %s on location %s") % (product.name, location.name)
                self.message_post(body=message)
        else:
            message = _("<b>Auto validation Failed</b> <br/> <b>Reason:</b> There are no Batches/Serial no's available for product %s") % product.name
            self.message_post(body=message)
            return False
    ...
    def validate_payment(self):
        for obj in self:
            ...
            default_vals = self.env['sale.advance.payment.inv'].with_context(ctx).default_get([
                'count', 'deposit_taxes_id', 'advance_payment_method', 'product_id', 'deposit_account_id'
            ])
            ...
            for inv in obj.invoice_ids:
                ...
                if inv.payment_state == 'paid':
                    ...
                    fields_keys = account_payment_env.fields_get().keys()
                    default_fields = account_payment_env.with_context({'default_invoice_ids': [(4, inv.id, None)]}).default_get(fields_keys)
                    ...
                    journal_payment_methods = journal_id.inbound_payment_method_line_ids.payment_method_id.ids if default_fields.get('payment_type') == 'inbound' else journal_id.outbound_payment_method_line_ids.payment_method_id.ids
                    common_payment_method = sorted(list(set(payment_method_ids).intersection(set(journal_payment_methods))))
                    ...
                    message = _("<b>Auto validation Failed</b> <br/> <b>Reason:</b> The Total amount is 0 So, Can't Register Payment.")
